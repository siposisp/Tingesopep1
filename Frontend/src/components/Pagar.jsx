...
const Pagar = () => {
    ...
    const handleCalculate = (patente) => {
        ...
        axios
            .get(`http://localhost:8080/historialreparaciones/calculate?patente=${patente}&bono=${bono}`)
            .then(() => {
    ...
    const handlePay = (patente) => {
        ...
        axios
            .get(`http://localhost:8080/historialreparaciones/pagar?patente=${patente}`)
            .then(() => {
            ...
            if (response.status === 200) {
                ...
                if (Array.isArray(data)) {
                    const reparacionesPromises = data.map(historial => 
                        axios.get(`http://localhost:8080/historialreparaciones/reparacion/historial/${historial.id}`)
                    );
                    ...
                    const reparacionesResponse = await axios.get(`http://localhost:8080/historialreparaciones/reparacion/historial/${data.id}`);
